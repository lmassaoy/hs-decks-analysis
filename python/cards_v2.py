...
st.text("Use the filters below to explore the dataframe")

st.header("**Filters**")

specific_card_text = st.text_input("Card Name (you can only type part of the name)")
...
class_multiselect = st.multiselect('Classes',list_class,list_class)
...
rarity_multiselect = st.multiselect('Rarity',list_rarity,list_rarity)
...
set_multiselect = st.multiselect('Set',list_set,list_set)
...
type_multiselect = st.multiselect('Type',type_rarity,type_rarity)
...
cost_slider = st.slider('Card Cost',min_card_cost,max_card_cost,[min_card_cost,max_card_cost],1)
